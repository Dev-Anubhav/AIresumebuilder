...
export default function DashboardHeader(_: DashboardHeaderProps) {
  ...
  return (
    <header className="border-b border-slate-800 bg-[#131b2e] shrink-0 z-20 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        {/* Brand */}
        <Link href="/dashboard" className="flex items-center gap-2.5 group">
          {/* Custom R logo mark */}
          <div
            className="w-9 h-9 rounded-xl flex items-center justify-center text-white shadow-lg shadow-primary/20 group-hover:scale-105 transition"
        ...
        <div className="flex items-center gap-3">
          {user && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-300 hidden md:inline-block truncate max-w-[120px]">
                {user.name}
              </span>
            </div>
          )}
          ...
          <button
            ...
            className="p-1.5 rounded-lg bg-slate-800 hover:bg-rose-950 text-slate-400 hover:text-rose-400 text-xs flex items-center gap-1.5 transition border border-slate-700 hover:border-rose-900"
            title="Log Out"
          >
